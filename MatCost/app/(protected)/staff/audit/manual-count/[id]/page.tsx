...
export default function StaffCountingPage() {
  ...
  // STATE MỚI: Dùng để bắt lỗi khi Backend từ chối cho xem danh sách (Vì đã nộp bài)
  ...
  const loadTasks = async () => {
    try {
        setLoading(true);
        setAccessDenied(false); // Reset trước khi gọi
        ...
        console.error("Access Denied by Backend:", error);
        // Nếu Backend văng lỗi (Từ chối truy cập), ta hiểu là Staff đã nộp bài xong!
...
const startEdit = (item: any, isRecount: boolean) => {
    ...
    showConfirmToast({
       ...
       onConfirm: async () => {
          try {
             ...
             // Thay vì đá về trang chủ, ta gọi lại loadTasks để nó tự bắt lỗi và hiện màn hình "Đã nộp"
             await loadTasks();
          } catch (error: any) {
  ...
  const filteredData = displayNormalData.filter(task => {
    ...
    return task.materialName.toLowerCase().includes(term) || 
           (task.batchCode && task.batchCode.toLowerCase().includes(term));
  });
  ...
  return (
    <div className="flex flex-row h-screen w-screen bg-slate-50/50">
      ...
      <main className="flex-grow flex flex-col overflow-hidden relative z-10">
        ...
        <div className="flex-1 w-full overflow-y-auto">
          {loading ? (
             <div className="flex flex-col items-center justify-center pt-32">
                ...
                <p className="text-slate-500 font-medium">Đang tải dữ liệu kiểm kê...</p>
             </div>
          ) : accessDenied ? (
             // BẢN VÁ UI: MÀN HÌNH "HOÀN THÀNH" ĐẸP MẮT KHI BỊ BACKEND TỪ CHỐI (ĐÃ NỘP BÀI)
             <div className="flex flex-col items-center justify-center pt-32 pb-20 space-y-5 px-4 animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <h2 className="text-3xl font-extrabold text-slate-800 text-center">Nhiệm vụ hoàn tất!</h2>
                  <p className="text-slate-600 text-center max-w-md text-base leading-relaxed">
                      Bạn đã nộp kết quả đếm cho phiếu kiểm kê này.<br/><br/>
                      Vui lòng chờ Quản lý kiểm tra. Nếu có chênh lệch, danh sách "Recount" sẽ tự động xuất hiện tại đây.
                  </p>
                  <Button size="lg" className="mt-6 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow-md h-12 px-8" onClick={() => router.push('/staff/audit')}>
                      <ArrowLeft className="w-5 h-5 mr-2" /> Quay về danh sách
                  </Button>
             </div>
          ) : (
            // GIAO DIỆN ĐẾM BÌNH THƯỜNG
              ...
              {activeTab === "normal" && (
                <div className="flex gap-3 items-center">
                    ...
                    <Button 
                      ...
                      className={`h-11 w-[140px] flex-shrink-0 transition-colors ${uncountedOnly ? "bg-indigo-600 text-white border-indigo-600" : "bg-white"}`}
                    >
                      <Filter className="w-4 h-4 flex-shrink-0" /> 
                      <span className="truncate">{uncountedOnly ? "Chưa đếm" : "Đã đếm"}</span>
                    </Button>
                </div>
              )}
              ...
              <div className="space-y-3">
                {activeTab === "recount" ? recountTasks.map(task => (
                    ...
                    <div className="text-center py-10 text-slate-400">
                      {searchTerm ? "Không tìm thấy mặt hàng nào." : "Danh sách trống."}
                    </div>
                    ...
                    <Card key={`${task.materialId}-${task.batchId}`} className={`border-l-4 ${uncountedOnly ? "border-l-indigo-400" : "border-l-emerald-500 bg-emerald-50/20"} shadow-sm`}>
                        <CardContent className="p-4 flex justify-between items-center">
                          ...
                          <Button size="sm" onClick={() => startEdit(task, false)} variant={uncountedOnly ? "default" : "outline"} className={uncountedOnly ? "bg-indigo-600 text-white" : ""}>
        ...
        {/* Ẩn khối Footer nếu bị Access Denied */}
        {!accessDenied && !loading && (
          <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t flex justify-center z-20">
            <Button size="lg" className={`w-full max-w-2xl font-bold ${progress < 100 || recountTasks.length > 0 ? "bg-slate-300 hover:bg-slate-300 text-slate-500" : "bg-emerald-600 hover:bg-emerald-700 text-white"}`} onClick={handleCompleteAudit} disabled={progress < 100 || recountTasks.length > 0}>
        ...
        <Dialog open={editingItem !== null} onOpenChange={(o) => !o && setEditingItem(null)}>
          <DialogContent className="sm:max-w-[400px]">
              ...
              <div className="py-4 space-y-4">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-semibold uppercase text-slate-500">Số lượng thực tế <span className="text-red-500">*</span></label>
              ...
              <DialogFooter>
                ...
                <Button onClick={saveCount} disabled={savingItem} className="bg-indigo-600 text-white hover:bg-indigo-700">{savingItem ? <Loader2 className="animate-spin w-4 h-4" /> : <Save className="w-4 h-4 mr-2"/>} Lưu</Button>
              </DialogFooter>
          </DialogContent>
        </Dialog>
      </main>
