...
import { 
  FileBarChart, FileSpreadsheet, FileWarning, 
  ArrowRight, Download, Printer, Bell, User, Loader2
} from "lucide-react";
...
import { toast } from "sonner"; // Giả lập toast
      ...
      <main className="flex-grow flex flex-col overflow-hidden relative z-10">
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-30 shadow-sm flex-shrink-0">
          <div className="bg-white px-6 lg:px-8 h-16 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-slate-900">Report Center</h2>
        ...
        <div className="flex-grow overflow-y-auto p-6 lg:p-10 space-y-8">
           ...
           <div>
              <h1 className="text-2xl font-bold text-slate-900">Available Reports</h1>
              <p className="text-sm text-slate-500">Select a report to view or export data.</p>
           </div>

           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {REPORTS.map((rpt) => (
                 <Card key={rpt.id} className="hover:shadow-md transition-shadow border-slate-200">
                    <CardContent className="p-6">
                       <div className="flex items-start gap-4 mb-4">
                          ...
                          <div>
                             <h3 className="font-bold text-lg text-slate-900">{rpt.title}</h3>
                             ...
                             <DialogContent>
                                   ...
                                   <div className="space-y-2">
                                      ...
                                      <Select defaultValue="all">
                                         ...
                                         <SelectContent>
                                            ...
                                            <SelectItem value="wh1">Central Storage</SelectItem>
                                         </SelectContent>
                                      </Select>
                                   </div>
                                </div>
                                <DialogFooter className="gap-2 sm:gap-0">
                                   ...
                                   <Button 
                                      ...
                                      disabled={isExporting}
                                   >
                                      {isExporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <FileSpreadsheet className="w-4 h-4 mr-2" />} 
                                      Excel
                                   </Button>
                                </DialogFooter>
                             </DialogContent>
