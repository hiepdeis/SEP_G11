...
type MaterialRow = { id: string; name: string; unit: string; unitPrice: number; qty: number; isEditing: boolean; };
...
export default function IssueMaterialPage() {
  ...
  const addRow = () => {
    const newMaterials = [...materials, { id: Date.now().toString(), name: "", unit: "", unitPrice: 0, qty: 1, isEditing: true }];
    ...
    setMaterials(materials.map(m => {
      if (m.id === id) {
        ...
        if (field === 'name') {
          ...
          if (found) { updated.unit = found.unit; updated.unitPrice = found.price; }
        }
        return updated;
      }
      return m;
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = startRow; i < aoa.length; i++) {
          ...
          extractedData.push({
            id: Date.now().toString() + i, name: String(row[1]).trim(), unit: row[2] ? String(row[2]).trim() : "",
            unitPrice: parseFloat(row[3]) || 0, qty: parseFloat(row[4]) || 0, isEditing: false
          });
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const slipPayload = {
        projectId: parseInt(formData.projectId), warehouseId: parseInt(formData.warehouseId),
        issueCode: formData.requestCode, userId: currentUserId, description: formData.notes,
        WorkItem: formData.workItem, Department: formData.department, DeliveryLocation: formData.deliveryLocation, ReferenceCode: formData.reference
      };
      ...
      const detailsPayload = materials.map(m => ({
         ...
         quantity: m.qty, unitPrice: m.unitPrice
      }));
        ...
        <div className="flex-grow overflow-y-auto p-6 lg:p-10 space-y-6 max-w-6xl mx-auto w-full">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button onClick={handleSubmit} disabled={isSubmitting} className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm">{isSubmitting ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />} {t("Submit Request")}</Button>
            </div>
          </div>

          <datalist id="material-suggestions">{MATERIAL_SUGGESTIONS.map(m => <option key={m.code} value={m.name} />)}</datalist>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="bg-slate-50">
                      ...
                      <TableHead className="w-[150px] text-right font-bold">{t("Unit Price")}</TableHead>
                      <TableHead className="w-[120px] text-right font-bold">{t("Quantity")}</TableHead>
                      <TableHead className="w-[150px] text-right font-bold">{t("Total Amount")}</TableHead>
                  ...
                  <TableBody>
                    {paginatedMaterials.length === 0 ? (
                      ...
                      paginatedMaterials.map((item, index) => (
                        <TableRow key={item.id} className="hover:bg-slate-50/50">
                          ...
                          <TableCell>{item.isEditing ? <Input list="material-suggestions" value={item.name} onChange={(e) => updateCell(item.id, 'name', e.target.value)} className="h-8 border-indigo-200" /> : <span className="font-medium text-slate-700">{item.name || "..."}</span>}</TableCell>
                          ...
                          <TableCell className="text-right">{item.isEditing ? <Input type="number" min={0} value={item.unitPrice} onChange={(e) => updateCell(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} className="h-8 text-right" /> : <span className="text-slate-600">{item.unitPrice.toLocaleString('vi-VN')}</span>}</TableCell>
                          ...
                          <TableCell className="text-right font-semibold text-emerald-600 bg-emerald-50/30">{(item.qty * item.unitPrice).toLocaleString('vi-VN')}</TableCell>
                    ...
                    {materials.length > 0 && (
                      <TableRow className="bg-slate-100/50">
                        <TableCell colSpan={5} className="text-right font-bold text-slate-800 uppercase tracking-wider">{t("Grand Total:")}</TableCell>
                        <TableCell className="text-right font-black text-emerald-700 text-lg">{grandTotal.toLocaleString('vi-VN')} VND</TableCell>
                        <TableCell></TableCell>
                      </TableRow>
                    )}
                  </TableBody>
