...
import { accountantPurchaseOrderApi, PurchaseOrderDto } from "@/services/import-service";
...
export default function AccountantPurchaseOrderListPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        // Dựa vào BE, API này lấy danh sách PO đang ở trạng thái "Draft" chờ duyệt
        const res = await accountantPurchaseOrderApi.getPendingOrders();
        setOrders(res.data);
      } catch (error) {
        console.error("Failed to fetch pending purchase orders", error);
  ...
  const filteredData = orders.filter((item) => {
    ...
    // API GetPendingOrders của bạn hiện tại chỉ trả về Draft, 
    // nhưng ta vẫn giữ filter ở đây để mở rộng về sau
  ...
  // Thống kê
  const pendingCount = orders.filter((item) => item.status === "Draft").length;
            ...
            <CardHeader className="border-b border-slate-100 pb-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 w-full">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Select
                    value={filterStatus}
                    onValueChange={(value: "All" | "Pending") => setFilterStatus(value)}
                  >
                ...
                <Table>
                  <TableHeader className="sticky top-0 z-20 bg-slate-50 shadow-sm outline outline-1 outline-slate-200">
                    <TableRow className="bg-slate-50">
                      ...
                      <TableHead className="text-right pr-6 w-[15%]">{t("Action")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {isLoading ? (
                      ...
                      paginatedData.map((item) => (
                        <TableRow key={item.purchaseOrderId} className="group hover:bg-slate-50/50 transition-colors">
                          <TableCell className="pl-6">
                            <div className="flex flex-col">
                              <span className="font-semibold text-slate-700">{item.purchaseOrderCode}</span>
                          ...
                          <TableCell>
                            <div className="flex flex-col text-left">
                              <span className="font-medium text-slate-800">{item.supplierName}</span>
                          ...
                          <TableCell className="text-right pr-6">
                            <Button
                              size="sm"
                              onClick={() => handleReview(item.purchaseOrderId)}
                            ...
                            >
                              {loadingId === item.purchaseOrderId ? (
                                ...
                                <>
                                  <Calculator className="w-4 h-4 mr-1.5" /> {t("Review Price")}
                                </>
                              )}
                            </Button>
                          </TableCell>
