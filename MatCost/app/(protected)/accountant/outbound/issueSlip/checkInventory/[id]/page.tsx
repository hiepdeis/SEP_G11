...
import {
  issueSlipApi,
  IssueSlipAllocation,
} from "@/services/issueslip-service";
import {
  ArrowLeft,
  Loader2,
  PackageSearch,
...
export default function CheckInventoryPage() {
  ...
  const [allocation, setAllocation] = useState<IssueSlipAllocation | null>(
    null,
  );
  ...
  const handleProcess = async () => {
    ...
    const items = allocation.items.map((item) => ({
      ...
      batchId:
        selectedBatches[item.detailId] || item.availableBatches[0]?.batchId,
      ...
      <div className="h-screen w-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-2 text-indigo-600">
          ...
          <p className="text-sm font-medium">
            {t("Loading inventory allocation...")}
          </p>
        </div>
      </div>
    );
  ...
  if (!allocation)
    return (
      <div className="p-10 text-center">{t("Allocation data not found")}</div>
    );
  ...
  return (
    <div className="flex flex-row h-screen w-screen overflow-hidden bg-slate-50/50">
      ...
      <main className="flex-grow flex flex-col overflow-hidden relative z-10">
        <Header
          title={`${t("Inventory Allocation")} #${allocation.issueCode}`}
        />
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50/50">
                        ...
                        <TableHead className="pr-6 w-[250px]">
                          {t("Select Batch (FIFO)")}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {allocation.items.map((item) => {
                        const currentBatch =
                          selectedBatches[item.detailId]?.toString() ||
                          item.availableBatches[0]?.batchId?.toString();
                        return (
                          <TableRow
                            key={item.detailId}
                          ...
                          >
                            ...
                            <TableCell className="text-right">
                              <span
                                className={`font-semibold ${!item.isEnough ? "text-rose-600" : "text-emerald-600"}`}
                              >
                                {item.totalAvailable}
                              </span>
                            </TableCell>
                            <TableCell className="pr-6">
                              {item.availableBatches.length === 0 ? (
                                <Badge
                                  variant="outline"
                                  className="text-rose-500 border-rose-200 bg-rose-50 w-full justify-center"
                                >
                  ...
                  {allocation.isAllEnough ? (
                    ...
                    <>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          toast.info(t("Feature under development"))
                        }
                      >
