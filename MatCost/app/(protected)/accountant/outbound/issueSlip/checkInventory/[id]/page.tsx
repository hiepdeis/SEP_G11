...
export default function CheckInventoryPage() {
  ...
  const handleBatchChange = (detailId: number, batchId: number) => {
    setSelectedBatches((prev) => ({ ...prev, [detailId]: batchId }));
  };
  ...
  const handleProcess = async () => {
    ...
    if (items.some(i => !i.batchId)) {
       toast.error(t("Shortage Detected! Cannot fulfill request."));
       return;
    }
    ...
    try {
      ...
      toast.success(t("Confirm & Process Issue"));
      router.push("/accountant/outbound/issueSlip"); 
    } catch (e: any) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s === "pending") return <Badge className="bg-amber-100 text-amber-800 border-none px-3 py-1"><AlertCircle className="w-4 h-4 mr-1.5" /> {t("Pending")}</Badge>;
    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-2 text-indigo-600">
          ...
          <p className="text-sm font-medium">{t("Loading inventory allocation...")}</p>
        </div>
      </div>
    );
  }

  if (!allocation) return <div className="p-10 text-center">{t("Allocation data not found")}</div>;

  return (
    <div className="flex flex-row h-screen w-screen overflow-hidden bg-slate-50/50">
      <Sidebar />
      <main className="flex-grow flex flex-col overflow-hidden relative z-10">
        <Header title={`${t("Inventory Allocation")} #${allocation.issueCode}`} />
              ...
              <Card className="border-slate-200 shadow-sm overflow-hidden gap-0">
                <CardHeader className="bg-white border-b border-slate-100 py-4">
                  <CardTitle className="text-base font-semibold flex items-center gap-2 text-slate-800">
                    <PackageSearch className="w-5 h-5 text-indigo-600" /> {t("Material Batches Allocation")}
                  </CardTitle>
                </CardHeader>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-slate-50/50">
                        <TableHead className="pl-6">{t("Material Name")}</TableHead>
                        <TableHead className="text-right">{t("Req Qty")}</TableHead>
                        <TableHead className="text-center">{t("Unit")}</TableHead>
                        <TableHead className="text-right">{t("Available Stock")}</TableHead>
                        <TableHead className="pr-6 w-[250px]">{t("Select Batch (FIFO)")}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                            ...
                            <TableCell className="pr-6">
                              {item.availableBatches.length === 0 ? (
                                <Badge variant="outline" className="text-rose-500 border-rose-200 bg-rose-50 w-full justify-center">
                                  <AlertTriangle className="w-3 h-3 mr-1" /> {t("Out of stock")}
                                </Badge>
                              ) : (
                                <Select value={currentBatch} onValueChange={(val) => handleBatchChange(item.detailId, parseInt(val))}>
                                  <SelectTrigger className="w-full bg-white"><SelectValue placeholder={t("Select batch")} /></SelectTrigger>
                                  <SelectContent>
            ...
            <div className="space-y-6">
              <Card className="border-slate-200 shadow-sm gap-0">
                ...
                <CardContent className="p-6 space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t("Issue Code")}</label>
                    <div className="mt-1 font-bold text-indigo-700">{allocation.issueCode}</div>
                  </div>
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t("Project Name")}</label>
                    <div className="mt-1 font-medium text-slate-800">{allocation.projectName || t("N/A")}</div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-indigo-200 shadow-sm bg-indigo-50/30 gap-0">
                <CardHeader className="border-b border-indigo-100 pt-4 pb-3">
                  <CardTitle className="text-base font-semibold flex items-center gap-2 text-indigo-800">
                    <ClipboardList className="w-5 h-5" /> {t("Accountant Action")}
                  </CardTitle>
                </CardHeader>
                  ...
                  {allocation.isAllEnough ? (
                    <>
                      <p className="text-sm text-indigo-700/80 mb-2">
                        {t("All materials have sufficient stock. Please review the selected batches and proceed.")}
                      </p>
                      <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-sm h-11" onClick={handleProcess} disabled={isProcessing}>
                        {isProcessing ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-2" />} {t("Confirm & Process Issue")}
                      </Button>
                    </>
                  ) : (
                    <>
                      <div className="p-3 bg-rose-50 border border-rose-200 rounded-md mb-2">
                        <p className="text-sm text-rose-700 flex items-start gap-1.5 font-medium">
                          ...
                          {t("Shortage Detected! Cannot fulfill request.")}
                        </p>
                      </div>
                      <Button className="w-full bg-amber-500 hover:bg-amber-600 text-white font-semibold shadow-sm h-11" onClick={() => toast.info(t("Feature under development"))}>
                        {t("Create Debt Slip / Partial Issue")}
                      </Button>
                    </>
                  )}
                </CardContent>
