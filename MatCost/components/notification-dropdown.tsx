...
const ROLES_CONFIG = [
  { id: "Manager", label: "Manager", path: "/manager", api: managerNotificationApi },
...
export function NotificationDropdown() {
  ...
  const totalUnread = useMemo(() => 
    Object.values(notifications).reduce((acc, list) => acc + list.length, 0), 
  [notifications]);
    ...
    const fetchAll = async () => {
      ...
      try {
        const results = await Promise.all(
          ROLES_CONFIG.map(role => role.api.getUnreadNotifications(0, 50))
        );
        
        const newNotifs = {} as Record<RoleId, NotificationDto[]>;
        ROLES_CONFIG.forEach((role, index) => {
          newNotifs[role.id] = results[index].data;
        });
  ...
  const handleNotificationClick = (notif: NotificationDto) => {
    ...
    const entityPath = ENTITY_URL_MAP[notif.relatedEntityType] || "notifications";
    
    const targetUrl = `${currentRoleConfig?.path}/${entityPath}/${notif.relatedEntityId}`;
