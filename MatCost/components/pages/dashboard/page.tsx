...
export default function DashboardPage() {
  ...
  const canViewMaterials = ["Admin", "Manager", "Staff"].some((r) =>
    userRole.includes(r),
  );
  ...
  const canViewIssues = ["Admin", "Manager", "Staff", "Construction"].some(
  ...
  const statsData: StatData[] = useMemo(() => {
    ...
    if (canViewMaterials) {
      ...
      stats.push({
        ...
        trendDirection:
          dashboardData.summary.lowStockCount > 0 ? "down" : "neutral",
        icon: Activity,
        theme: "red",
      });
    }
  ...
  const quickLinks = [
    ...
    },
    {
      ...
      roles: ["Purchasing", "Accountant", "Admin"],
      icon: Users,
    },
    {
      label: "Projects",
    ...
    },
    {
      ...
      roles: ["Admin", "Purchasing", "Accountant"],
      icon: FileText,
    },
    {
      label: "Issue Slips",
    ...
    (link) =>
      !link.roles ||
      link.roles.some((r) => userRole.toLowerCase().includes(r.toLowerCase())),
  );
  ...
  const getStatusBadgeTheme = (status: string) => {
    ...
    if (s.includes("forwarded") || s.includes("chuyển"))
      return "bg-blue-100 text-blue-700";
    if (s.includes("rejected") || s.includes("hủy"))
          ...
          <div className="px-6 lg:px-10 py-8 mx-auto w-full">
            ...
            {loading ? (
              ...
              <>
                        ...
                        <div className="w-full h-full p-6 lg:p-8 bg-gradient-to-br from-indigo-600 via-purple-600 to-indigo-800 flex items-center relative">
                          ...
                          <div className="relative z-10 flex flex-col gap-3">
                            ...
                            <p className="text-indigo-100 text-base lg:text-lg max-w-2xl leading-relaxed">
                              Welcome to your MatCost dashboard. You currently
                              have access to {quickLinks.length} specific
                              modules based on your role.
                            </p>
                            {quickLinks.length > 0 && (
                              <div className="mt-2 flex items-center gap-3">
                                <Button
                                  variant="secondary"
                                  className="bg-white text-indigo-700 hover:bg-slate-50 border-none font-semibold shadow-sm"
                                  onClick={() =>
                                    router.push(quickLinks[0].href)
                                  }
                                >
                ...
                {/* Main Content Grid */}
                <div
                  className={`grid grid-cols-1 ${canViewMaterials ? "lg:grid-cols-2" : ""} gap-6 mb-8`}
                >
                  {canViewMaterials && (
                    <Card>
                  ...
                  <div
                    className={`flex flex-col gap-6 ${!canViewMaterials ? "lg:flex-row items-stretch" : ""}`}
                  >
                    <Card
                      className={`flex flex-col ${!canViewMaterials ? "flex-1" : ""}`}
                    >
                      <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-semibold text-slate-900">
                          Quick Access
                      ...
                      <div
                        className={`grid gap-4 ${quickLinks.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
                      >
                    ...
                    {(canViewReceipts || canViewIssues) && (
                      <Card
                        className={`flex flex-col pb-0 ${!canViewMaterials ? "flex-1" : ""}`}
                      >
                            ...
                            <div className="relative pb-6">
                              ...
                              <div className="space-y-4 relative z-10">
                                {recentActivities.map((activity, i) => (
                                  ...
                                  >
                                    <div
                                      className={`p-2 rounded-full shrink-0 ${activity.iconBg} ring-4 ring-white shadow-sm transition-all group-hover:scale-110 mt-1`}
                                    >
                                    ...
                                    <div className="flex-grow bg-white border border-slate-100 rounded-xl p-3 shadow-sm group-hover:border-indigo-100 transition-colors">
                                      ...
                                      <span
                                        className={`inline-flex px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${getStatusBadgeTheme(activity.status)}`}
                                      >
                                        {activity.status}
                                      </span>
                                    </div>
