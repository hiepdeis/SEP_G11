...
import { Search, Loader2, FileText, ChevronLeft, ChevronRight, CheckCircle2, ArrowUp, ArrowDown, ArrowUpDown, CalendarDays, Warehouse as WarehouseIcon, ClipboardList, Delete, Calendar as CalendarIcon, Clock, Filter } from "lucide-react";
...
export default function AuditReportPage({ role = "admin" }: Props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const completedAudits = data.filter(a => a.status.toLowerCase() === "completed");
  ...
  useEffect(() => { setCurrentPage(1); }, [searchTerm, itemsPerPage, sortConfig, dateRange]);
  ...
  const latestAudit = audits.length > 0 
    ? new Date(Math.max(...audits.map(a => new Date(a.plannedStartDate).getTime())))
    : null;
        ...
        <div className="flex-grow overflow-y-auto p-6 lg:p-10 space-y-6">
          <div className="flex justify-between items-center">
            <div className="flex flex-col gap-1">
              <h1 className="text-2xl font-bold tracking-tight text-slate-900">{t("Completed Audit Reports")}</h1>
              <p className="text-sm text-slate-500">{t("View detailed records of finished inventory audits.")}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="bg-white border-slate-200 shadow-sm"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-emerald-100 text-emerald-600 rounded-lg"><CheckCircle2 className="w-6 h-6" /></div><div><p className="text-sm font-medium text-slate-500">{t("Total Completed")}</p><h3 className="text-2xl font-bold text-slate-900">{totalCompleted}</h3></div></CardContent></Card>
            <Card className="bg-white border-slate-200 shadow-sm"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-blue-100 text-blue-600 rounded-lg"><WarehouseIcon className="w-6 h-6" /></div><div><p className="text-sm font-medium text-slate-500">{t("Warehouses Audited")}</p><h3 className="text-2xl font-bold text-slate-900">{warehouseCount}</h3></div></CardContent></Card>
            <Card className="bg-white border-slate-200 shadow-sm"><CardContent className="p-4 flex items-center gap-4"><div className="p-3 bg-amber-100 text-amber-600 rounded-lg"><Clock className="w-6 h-6" /></div><div><p className="text-sm font-medium text-slate-500">{t("Latest Completion")}</p><h3 className="text-2xl font-bold text-slate-900">{latestAudit ? format(latestAudit, "dd/MM/yyyy") : t("N/A")}</h3></div></CardContent></Card>
          </div>
                ...
                <Table className="w-full min-w-[800px] table-fixed">
                  <TableHeader className="sticky top-0 z-20 bg-slate-50 shadow-sm outline outline-1 outline-slate-200">
                    <TableRow className="bg-slate-50 hover:bg-slate-50">
                      <TableHead className="pl-6 w-[30%] cursor-pointer select-none group" onClick={() => handleSort("title")}>
                        <div className="flex items-center gap-1.5 hover:text-slate-800 transition-colors">
                          {t("Audit Info")} {getSortIcon("title")}
                        </div>
                      </TableHead>
                      <TableHead className="w-[20%] cursor-pointer select-none group" onClick={() => handleSort("warehouse")}>
                        <div className="flex items-center gap-1.5 hover:text-slate-800 transition-colors">
                          {t("Warehouse")} {getSortIcon("warehouse")}
                        </div>
                      </TableHead>
                      <TableHead className="w-[18%] cursor-pointer select-none group" onClick={() => handleSort("date")}>
                        <div className="flex items-center gap-1.5 hover:text-slate-800 transition-colors">
                          {t("Completed Date")} {getSortIcon("date")}
                        </div>
                      </TableHead>
                      <TableHead className="w-[17%] cursor-pointer select-none group" onClick={() => handleSort("progress")}>
                  ...
                  <TableBody>
                    {loading ? (
                      <TableRow><TableCell colSpan={5} className="h-32 text-center"><div className="flex justify-center items-center gap-2 text-indigo-600"><Loader2 className="w-6 h-6 animate-spin" /> {t("Loading reports...")}</div></TableCell></TableRow>
                    ) : paginatedData.length === 0 ? (
                      <TableRow><TableCell colSpan={5} className="h-32 text-center text-slate-500 border-b-0"><div className="flex flex-col items-center justify-center gap-2"><FileText className="w-8 h-8 text-slate-300" /><p>{t("No completed audits found matching your filters.")}</p></div></TableCell></TableRow>
                    ) : (
                        ...
                        >
                          ...
                          <TableCell className="text-right pr-6">
                             <Button 
                              ...
                              onClick={(e) => { e.stopPropagation(); navigateToDetail(audit.stockTakeId); }}
                            >
                              {t("Detail")}
                            </Button>
                          </TableCell>
