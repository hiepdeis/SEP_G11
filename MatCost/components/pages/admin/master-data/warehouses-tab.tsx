...
import { WarehouseRow } from "@/lib/master-data-types";
...
export function WarehousesTab({
  ...
  const [editing, setEditing] = useState<Partial<WarehouseRow> | null>(null);
  ...
  useEffect(() => {
    getWarehouses()
      ...
      .catch((err: any) => {
        toast.error(
          err.response?.data?.message || t("Failed to load warehouses"),
        );
        setLoading(false);
      });
  }, [t]);
  ...
  const openEdit = (item: WarehouseRow) => {
    setEditing({ ...item });
    setModalOpen(true);
  };
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      if (editing._id) {
        await updateWarehouse(editing._id, { name, address });
        setItems((prev) =>
          prev.map((i) =>
            i._id === editing._id ? { ...i, name, address } : i,
          ),
        );
        toast.success(t("Update Successful"));
      } else {
        const created = await createWarehouse({ name, address });
        setItems((prev) => [...prev, { _id: created.id, name, address }]);
